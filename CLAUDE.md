...
- **Phase 2**(質問設計):完了。**16P式スケール × 28問**(正本=`app/js/questions.js`)
- **Phase 3**(タイプ詳細解説):全16タイプ「これ私すぎる」水準。正本=`app/js/data.js`
- **Phase 4**(レコメンドエンジン):**748曲 × 11リスナー層(cluster)× 16タイプスコア × mood多様化**(2026-06-22 進化)。Spotify Web API 本接続済(Vercel Serverless Functions)
- **Phase 5**(ビジュアル):**全身SVGキャラ×16**(`app/js/mascot.js`)+ タイプ別カラー(結果ページ全panelに展開)+ **正式キャラAI生成はファウンダー側で進行予定**
- **Phase 6**(実装):**ほぼ完成形**。Vercel auto-deploy(`https://lovesong-type16.vercel.app/`)+ **PWA対応(ホームに追加でアプリ起動)**
- **Phase 7〜8**(テストプレイ・本番公開):未着手

### プロトタイプ現状(2026-06-22 完成形)
- 見た目=明るいポップ系(白基調・丸み・Zen Maru Gothic + Yomogi・シール装飾)
- 回答=**16P式 7段階スケール × 28問**(逆方向重みあり、touch vibrate)
- 楽曲=**710曲ライブラリ**(2026-06-23 非ラブソング/誤データ整理) × 16タイプスコア × **14リスナー層(cluster)**
- **cluster**:JPOP_MAIN / INDIE_ROCK / ROCK_CLASSIC / KPOP / ANISON_VOCALO / HIPHOP_RNB / DIVA_BALLAD / CITYPOP / WPOP / JOHNNYS(男性アイドル) / IDOL_F(女性アイドル) / SHOWA / HEISEI / REIWA(世代別)
- **GENRES(15)**:昭和歌謡曲 / 平成ソング / 令和J-POP / K-POP / 洋楽 / 邦ロック / ボカロ / シティポップ・R&B / 日本語ラップ・HIPHOP / EDM・ダンスミュージック / アニソン / バラード / 女性アイドル / 男性アイドル
- レコメンド=**3層guarantee**(好きアーティスト2曲 + リスナー層一致3曲 + mood多様化5曲)
- 結果=「◯◯系 × タイプ」二層命名(64通り)+ 20panelスクロールリビール + 16タイプ図鑑
- 演出=**判定中1.5sローディング演出** / リビールpanel(粒子12+aurora) / 各panel順次表示 / マスコットpulse / mood pillホバー強調 / 試聴jacket回転+波形bar
- 拡散仕掛け=**標準PNG + Stories向け縦長 9:16** / X/LINE/Insta/TikTok/BeReal / 相性シェア / 再シャッフル / 「もう一度診断する」リセット動線
- インフラ=**PWA対応**(manifest + service worker + 「ホームに追加」standalone) / **sticky CTA bar**(モバイル下部固定) / **state localStorage resume**(24h) / **A11y**(role/aria/focus-visible)
- 監査=`npm run audit`(`scripts/audit-songs.py`)で重複/cluster/mood/flat/タイプ充足度を一括チェック
- **未実装(ファウンダー判断必須)**:正式キャラAI生成(進行予定)/ 信頼感の本物化(監修者プロフ・実績バッジ・診断数実データ)/ 法務(プラポリ・利用規約)/ 独自ドメイン取得
